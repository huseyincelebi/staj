...
<body>
    ...
    <div class="container">
        <div class="row mx-auto vh-100">
            ...
            <div class="col-md-7 p-4">
                <div class="row my-2">
                    <div class="col">
                        <p class="lead fs-4 text-decoration-none text-dark"><strong>Soruyu Düzenle</strong></p>
                        <form action="../../admin/questions/edit" method="POST" name="answer_question_form">
                            <div class="input-group mb-3">
                                <span class="input-group-text" id="basic-addon1">Başlık</span>
                                <input type="text" class="form-control" name="q_e_title" value="<?= $questions_edit['title'] ?>">
                            </div>
                            <div class="input-group mb-3">
                                <span class="input-group-text" id="basic-addon1">Tag</span>
                                <input type="text" class="form-control" id="q_e_token" name="q_e_token" data-limit=3>
                            </div>
                            <input name="q_e_id" id="q_e_id" value=<?= $questions_edit['id'] ?> hidden>
                            <input name="q_e_data" id="q_e_data" hidden>
                            ...
                            <div class="input-group p-2" id="t_editor_q_e" contentEditable="true" style="display: inline-block;border:1px solid #ced4da;min-height:240px;resize: vertical;overflow: auto;">
                                <?= $questions_edit['content'] ?>
                            </div>
                            <div class="d-flex justify-content-end">
                                <button type="submit" class="btn btn-primary my-2 w-25" name="edit_question">Düzenle</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        ...
        function add_image_answer() {
            ...
            $.ajax(settings).done(function(response) {
                var api = JSON.parse(response);
                $('#t_editor_q_e').append("<img src=" + api.data.url + " style='max-width:650px;max-height:400px'>");
                $('#q_e_data').val($('#t_editor_q_e').html().trim());
            });
        }
    </script>
    <script>
    ...
    <?php if (count($edit_tags) == 3) : ?>
        <script>
            $(document).ready(function() {
                $('#q_e_token').tokenfield({
                    autocomplete: {
                        source: '../../post/fetch_all_tags',
                        delay: 100
                    }
                });
                $('#q_e_token').tokenfield('setTokens', '<?= $tags[array_search($edit_tags[0]['tag_id'], array_column($tags, 'id'))]['name']; ?>,<?= $tags[array_search($edit_tags[1]['tag_id'], array_column($tags, 'id'))]['name']; ?>,<?= $tags[array_search($edit_tags[2]['tag_id'], array_column($tags, 'id'))]['name']; ?>');
            });
        </script>
    <?php elseif (count($edit_tags) == 2) : ?>
        <script>
            $(document).ready(function() {
                $('#q_e_token').tokenfield({
                    autocomplete: {
                ...
                $('#q_e_token').tokenfield('setTokens', '<?= $tags[array_search($edit_tags[0]['tag_id'], array_column($tags, 'id'))]['name']; ?>,<?= $tags[array_search($edit_tags[1]['tag_id'], array_column($tags, 'id'))]['name']; ?>');
            });
        </script>
    <?php else : ?>
        <script>
            $(document).ready(function() {
                ...
                $('#q_e_token').tokenfield('setTokens', '<?= $tags[array_search($edit_tags[0]['tag_id'], array_column($tags, 'id'))]['name']; ?>');
            });
        </script>
    <?php endif; ?>
    <script>
        $(document).ready(function() {
            ...
            $('#t_editor_q_e').on('input', function() {
                $('#q_e_data').val($('#t_editor_q_e').html().trim());
            })
        });
    </script>
