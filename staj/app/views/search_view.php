    ...
    <div class="container">
        <div class="row mx-auto vh-100">
            ...
            <div class="col-md-7 p-4">
                <div class="row">
                    <div class="col">
                        <h1 class="lead fs-3"><strong>Arama Sonuçları</strong></h1>
                    </div>
                </div>
                <div class="row mt-4 align-items-center">
                    <div class="col">
                        <h1 class="lead fs-6"><strong>Toplam <?= count($questions_count) ?> sonuç bulundu</strong></h1>
                    </div>
                    <div class="col d-flex justify-content-end">
                        <div class="btn-group" role="group" aria-label="Basic outlined example">
                            <?php if (isset($_GET['page'])) : ?>
                                <a href="<?= parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH) . '?page=' . $_GET['page']; ?>&filter=newest&title=<?= $_GET['title'] ?>" type="button" class="btn btn-primary">En&nbsp;Yeni</a>
                                <a href="<?= parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH) . '?page=' . $_GET['page']; ?>&filter=oldest&title=<?= $_GET['title'] ?>" type="button" class="btn btn-outline-primary">En&nbsp;Eski</a>
                                <a href="<?= parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH) . '?page=' . $_GET['page']; ?>&filter=popular&title=<?= $_GET['title'] ?>" type="button" class="btn btn-outline-primary">En&nbsp;Popüler</a>
                                <a href="<?= parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH) . '?page=' . $_GET['page']; ?>&filter=unanswered&title=<?= $_GET['title'] ?>" type="button" class="btn btn-outline-primary">Yanıtlanmamış</a>
                            <?php else : ?>
                                ...
                                <a href="?filter=popular" type="button" class="btn btn-outline-primary">En&nbsp;Popüler</a>
                                <a href="?filter=unanswered" type="button" class="btn btn-outline-primary">Yanıtlanmamış</a>
                            <?php endif; ?>
                        </div>
                    </div>
                    <hr class="mt-3">
                        ...
                        <?php if (!isset($_GET['filter'])) : for ($page = 1; $page <= $pages; $page++) : ?>
                                <?php if (!isset($_GET['page'])) : if ($page == 1) : ?>
                                        <li class="page-item active" aria-current="page">
                                            <span class="page-link"><a class="text-decoration-none text-white" href="?page=<?= $page ?>&title=<?= $_GET['title'] ?>"><?= $page ?></a></span>
                                        </li>
                                    <?php endif;
                                    if ($page != 1) : ?>
                                        <li class="page-item" aria-current="page">
                                            <span class="page-link"><a class="text-decoration-none text-primary" href="?page=<?= $page ?>&title=<?= $_GET['title'] ?>"><?= $page ?></a></span>
                                        </li>
                                    <?php endif;
                            ...
                            <?php for ($page = 1; $page <= $pages; $page++) : ?>
                                <?php if (!isset($_GET['page'])) : if ($page == 1) : ?>
                                        <li class="page-item active" aria-current="page">
                                            <span class="page-link"><a class="text-decoration-none text-white" href="?page=<?= $page ?>&filter=<?= $_GET['filter'] ?>&title=<?= $_GET['title'] ?>"><?= $page ?></a></span>
                                        </li>
                                    <?php endif;
                                    if ($page != 1) : ?>
                                        <li class="page-item" aria-current="page">
                                            <span class="page-link"><a class="text-decoration-none text-primary" href="?page=<?= $page ?>&filter=<?= $_GET['filter'] ?>&title=<?= $_GET['title'] ?>"><?= $page ?></a></span>
                                        </li>
                                    <?php endif;
                                else : if ($page == $_GET['page']) : ?>
                                        <li class="page-item active" aria-current="page">
